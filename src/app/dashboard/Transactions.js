...
const DetailsModal = ({values, handleState, open}) => {
  ...
  function updateTxnStatus(id, status) {
    let url = '/.netlify/functions/updateTransaction';
    let body = {
    ...
    .then(data => {
      console.log(data);
      displayData.confirmation_status = !displayData?.confirmation_status
      setSuccessMsg('Status updated successfully!')
    })
    .catch(error => {
      console.log(error.message)
      setErrMsg('Could not update status')
    })
  }
    ...
    <>
      <Modal show={open} onHide={handleClose}>
        ...
        <Modal.Footer>
          ...
          <Button variant="primary" onClick={() => {updateTxnStatus(displayData?.txn_id, !displayData?.confirmation_status)}}>
            Set {displayData?.confirmation_status ? 'Pending': 'Active'}
          </Button>
        </Modal.Footer>
      </Modal>
    </>
